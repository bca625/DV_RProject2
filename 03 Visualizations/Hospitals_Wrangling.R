...
# select
Texas <- hospitals %>% select(CITY, STATE, PAYMENT) %>% filter(STATE == "TX") %>% tbl_df
HSD <- hospitals %>% select(CITY, STATE, PAYMENT) %>% filter(STATE == "TX", CITY %in% c("HOUSTON", "SAN ANTONIO", "DALLAS" ))
